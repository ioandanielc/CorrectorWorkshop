...
-----
    ...
    cfg = KDTreeCorrector2DConfig.from_yaml('src/configs/experiments/sph_tv/model12_kdtree.yaml')
...
class _KDTreeCorrectorND(Corrector):
    ...
    def _load_model(self):
        ...
        # See grid/corrector.py's _load_model for the attention_rd rationale —
        # same adapter, same reasoning, kept in sync deliberately.
    ...
    def apply(self, points: np.ndarray, k: int = 1) -> np.ndarray:
        ...
        for _ in range(k):
            ...
            x_t = torch.tensor(batch, dtype=torch.float32, device=self.device)
            with torch.no_grad():
