"""
sph_tv_experiment.py
---------------------
ML corrector vs. Transport-Velocity baseline on the SPH trajectory.

Creates an experiment directory mirroring the artifacts/training structure:

  artifacts/inference/experiments/sph_tv/runs/exp_YYYY-MM-DD_HH-MM-SS/
...
-----
    ...
    # Override stride
    .venv\\Scripts\\python.exe src/inference/experiments/sph_tv/sph_tv_experiment.py src/configs/experiments/sph_tv/grid_6x6.yaml --stride 50
"""
import argparse
import logging
import shutil
...
from inference.correctors.tv.tv_corrector import TVCorrector2D, FastTVCorrector2D
from utils.visualizations.inference_visualizations.comparison import plot_comparison_frame
from utils.visualizations.inference_visualizations.timeseries import plot_timeseries
...
class SPHTVExperiment(Experiment):
    """
    Runs the ML corrector (across k_values) and the TV baseline over the
    no-TV SPH trajectory, and saves comparison plots + a report.
    ...
    def __init__(self, config_path: str, timestep: Optional[int] = None,
        ...
        self.stride   = stride if stride is not None else int(exp_raw.get('stride', 100))
        self.k_values = list(exp_raw.get('k_values', [1, 2, 3, 5]))
    ...
    def run(self) -> None:
        ...
        exp_dir = Path('artifacts/inference/experiments/sph_tv/runs') / f'exp_{tag}'
        exp_dir.mkdir(parents=True)
        frames_dir = exp_dir / 'frames'
        frames_dir.mkdir()

        shutil.copy(self.config_path, exp_dir / 'config.yaml')
        ...
        # ── load data + corrector ────────────────────────────────────────────
        data_raw    = self.raw.get('data', {})
        pos_without = np.load(data_raw.get('without_tv', 'artifacts/inference/experiments/sph_tv/data/positions_without.npy'))
        pos_with    = np.load(data_raw.get('with_tv',    'artifacts/inference/experiments/sph_tv/data/positions.npy'))
        ...
        tv_cfg = self.raw.get('tv', {})
        tv_kw = dict(
            h_factor = float(tv_cfg.get('h_factor', 1.3)),
        ...
        if self.timestep is not None:
            timesteps = [self.timestep % T]
        else:
            timesteps = list(range(0, T, self.stride))

        ts_list          = []
        metrics_by_k     = {k: {'mean': [], 'cv': []} for k in self.k_values}
        metrics_tv       = {'mean': [], 'cv': []}
        metrics_tv_algo  = {'mean': [], 'cv': []}
        timings_k        = {k: [] for k in self.k_values}   # collect per-timestep for median
        timings_tv_fast  = []
        t_naive_ref      = None
        ...
        if len(ts_list) > 1 and timings_k[self.k_values[0]]:
...
if __name__ == '__main__':
    ...
    SPHTVExperiment(args.config, timestep=args.timestep, stride=args.stride).run()
