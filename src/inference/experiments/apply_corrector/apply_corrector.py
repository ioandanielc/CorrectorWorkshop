...
Output
------
  artifacts/inference/experiments/apply_corrector/runs/positions_corrected_K{k}.npy   shape (T, N, 2) float32
  artifacts/inference/experiments/apply_corrector/runs/positions_corrected_K{k}.txt   human-readable, one block per timestep

Usage
-----
    .venv\\Scripts\\python.exe src/inference/experiments/apply_corrector/apply_corrector.py
    .venv\\Scripts\\python.exe src/inference/experiments/apply_corrector/apply_corrector.py --k 3
    .venv\\Scripts\\python.exe src/inference/experiments/apply_corrector/apply_corrector.py --config src/configs/experiments/apply_corrector/grid_6x6.yaml --k 5
"""
import argparse
import sys
import time
...
def main():
    ...
    parser.add_argument('--config', default='src/configs/experiments/apply_corrector/grid_6x6.yaml')
    ...
    pos_without = np.load('artifacts/inference/experiments/sph_tv/data/positions_without.npy')
    ...
    # ── save ──────────────────────────────────────────────────────────────────
    out_dir = Path('artifacts/inference/experiments/apply_corrector/runs')
    ...
    with open(txt_out, 'w') as f:
        ...
        f.write(f'# Source: artifacts/inference/experiments/sph_tv/data/positions_without.npy\n')
