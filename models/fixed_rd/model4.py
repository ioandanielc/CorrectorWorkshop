"""
model4: model1 without centroid subtraction in forward().

Changes vs model1:
- Centroid subtraction removed — DataProcessor.make_invariant() handles it upstream,
  doing it again here is redundant (same bug that model2/3 also fixed).
- Everything else is identical to model1: BatchNorm, concatenative skip (hidden_dim*2),
  no additive residuals, no LayerNorm, no output bounding.

Intended to run on GPU with larger hidden_dim / batch than the model1 baseline.
"""
import torch
