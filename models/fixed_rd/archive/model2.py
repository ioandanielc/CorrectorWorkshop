"""
model2: model1 + additive residuals + centroid removal moved to data pipeline.

Changes vs model1:
- Removed centroid subtraction in forward() — DataProcessor handles this
- Each attention block now adds an additive residual:
    x = concat_mlp(cat([x, attended])) + x
  This preserves the concatenative skip connection while also adding a gradient
  highway through the additive path.
"""
import torch
...
class CorrectorModel(nn.Module):
    ...
    def forward(self, x, return_attention_maps=False):
        ...
        for query_mlp, key_mlp, value_mlp, concat_mlp in self.attention_modules:
            ...
            x = self._apply_module(torch.cat([x, attended], dim=-1), concat_mlp) + residual
